...
BTN_START_RECT = pygame.Rect(CENTER_X, CENTER_Y - 180, BTN_WIDTH, BTN_HEIGHT)
BTN_WHITE_RECT = pygame.Rect(CENTER_X, CENTER_Y - 70, BTN_WIDTH, BTN_HEIGHT)
BTN_BLACK_RECT = pygame.Rect(CENTER_X, CENTER_Y + 40, BTN_WIDTH, BTN_HEIGHT)

# Difficulty Buttons (Row)
DIFF_Y = CENTER_Y + 180
DIFF_WIDTH = 110
DIFF_HEIGHT = 60
BTN_EASY_RECT = pygame.Rect(CENTER_X - 15, DIFF_Y, DIFF_WIDTH, DIFF_HEIGHT)
BTN_MED_RECT = pygame.Rect(CENTER_X + 105, DIFF_Y, DIFF_WIDTH, DIFF_HEIGHT)
BTN_HARD_RECT = pygame.Rect(CENTER_X + 225, DIFF_Y, DIFF_WIDTH, DIFF_HEIGHT)
BTN_CLOCK_RECT = pygame.Rect(CENTER_X + 345, DIFF_Y, 150, DIFF_HEIGHT)
BTN_THEME_RECT = pygame.Rect(CENTER_X - 10, DIFF_Y + 80, BTN_WIDTH + 20, 70)
            ...
            for c in range(8):
                ...
                if piece:
                    ...
                    if piece.color == 'black':
                        pst_val = [pst for pst in globals().get(piece.__class__.__name__.upper() + '_PST')][7-r][c]
                        score -= (val + pst_val)
                    else:
                        score += (val + pst_val)
        return score
...
def draw_menu(win, font, current_difficulty):
    ...
    draw_diff_btn(BTN_HARD_RECT, "Hard", 4)
    ...
    t_surf = diff_font.render(f"Change Board (Theme: {theme_name})", True, WHITE_COLOR)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if state == MENU:
                        ...
                        if BTN_EASY_RECT.collidepoint(event.pos):
                            ...
                            print("Difficulty set to Hard (Depth 4 + Positional AI)")
                            ...
                            globals()['current_theme_index'] = (globals().get('current_theme_index', 0) + 1) % len(BOARD_THEMES)
                            print(f"Theme changed to: {BOARD_THEMES[globals()['current_theme_index']][2]}")
