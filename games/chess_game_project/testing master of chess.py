...
# Ensure current_theme_index is valid after modifying BOARD_THEMES
if current_theme_index >= len(BOARD_THEMES):
    current_theme_index = 0
...
CENTER_Y = HEIGHT // 2 - 60 # Moved up by 60 pixels
...
DIFF_Y = CENTER_Y + 40 # Adjusted to new CENTER_Y
DIFF_WIDTH = 85
DIFF_HEIGHT = 50
...
class Piece:
    ...
    def get_valid_moves(self, pos, board, current_ep_target=None): # Modified signature
    ...
    def get_valid_moves(self, pos, board, current_ep_target=None): # Modified signature
        ...
        for dc in [-1, 1]:
            if 0 <= r + direction < 8 and 0 <= c + dc < 8:
                ...
                # En Passant - use the provided current_ep_target
                if current_ep_target: 
                    ep_r, ep_c = current_ep_target
...
class AI:
    def __init__(self, depth=2):
        ...
        # AI's internal en passant target for simulation
        self.ai_simulation_en_passant_target = None # Initialize

        # Evaluation weights (new constants for mobility and king safety)
        self.MOBILITY_WEIGHT_PER_MOVE = 1.5 # Points per legal move
        self.PAWN_SHIELD_SCORE = 15        # Score for an intact friendly pawn in front of the king
        self.PAWN_SHIELD_PENALTY_NO_PAWN = -20 # Penalty for an empty square in front of king
        self.PAWN_SHIELD_PENALTY_MOVED_PAWN = -10 # Penalty for a non-pawn or enemy piece in front of king
    ...
    def evaluate_board(self, board, game_ref=None):
        ...
        # Iteration 1: Material, PST, collect king positions and raw mobility counts
        for r in range(8):
            for c in range(8):
                piece = board[r][c]
                if piece:
                    ...
                    if isinstance(piece, Pawn): pst_val = PAWN_PST[r][c]
                        ...
                        pst_val = KING_PST[r][c]
                        if piece.color == 'white': white_king_pos = (r, c)
                        else: black_king_pos = (r, c)
                    
                    # Apply PST and Material
                    if piece.color == 'black':
                        pst_name = piece.__class__.__name__.upper() + '_PST'
                        pst_table = globals().get(pst_name)
                        # Mirror PST for black pieces
                        pst_val_adjusted = pst_table[7-r][c] if pst_table else 0
                        score -= (val + pst_val_adjusted)
                    else:
                        score += (val + pst_val)
                
                    # --- Basic Mobility Count ---
                    # Only calculate if game_ref is available and not in a game_over state (for efficiency)
                    # Note: Calling get_legal_moves for every piece in every evaluation is expensive.
                    # This fulfills the request but could be optimized further if performance becomes a critical issue.
                    if game_ref: 
                        # Pass the AI's simulated en passant target for mobility calculation
                        num_moves = len(game_ref.get_legal_moves(piece, (r, c), board, ep_target_for_check=self.ai_simulation_en_passant_target))
                        if piece.color == 'white':
                            white_mobility += num_moves
                        else:
                            black_mobility += num_moves
        
        # Iteration 2: Apply Mobility Scores
        score += white_mobility * self.MOBILITY_WEIGHT_PER_MOVE
        score -= black_mobility * self.MOBILITY_WEIGHT_PER_MOVE

        # Iteration 3: King Safety (Pawn Shield)
        # White King Safety
        if white_king_pos:
            wk_r, wk_c = white_king_pos
            king_safety_score = 0
            
            # Check squares directly one rank in front of the white king
            for dc_offset in [-1, 0, 1]: # Columns relative to king's file
                pawn_r = wk_r - 1 # One rank above (lower row index for white)
                pawn_c = wk_c + dc_offset
                
                if 0 <= pawn_r < 8 and 0 <= pawn_c < 8: # Ensure square is on board
                    p = board[pawn_r][pawn_c]
                    if isinstance(p, Pawn) and p.color == 'white':
                        king_safety_score += self.PAWN_SHIELD_SCORE # Intact friendly pawn
                    elif p is None:
                        king_safety_score += self.PAWN_SHIELD_PENALTY_NO_PAWN # Empty square, a major weakness
                    else: # Other piece (enemy or friendly non-pawn)
                        king_safety_score += self.PAWN_SHIELD_PENALTY_MOVED_PAWN # Blocked or altered shield
            
            score += king_safety_score

        # Black King Safety (mirrored logic)
        if black_king_pos:
            bk_r, bk_c = black_king_pos
            king_safety_score = 0
            
            # Check squares directly one rank in front of the black king
            for dc_offset in [-1, 0, 1]: # Columns relative to king's file
                pawn_r = bk_r + 1 # One rank below (higher row index for black)
                pawn_c = bk_c + dc_offset
                
                if 0 <= pawn_r < 8 and 0 <= pawn_c < 8: # Ensure square is on board
                    p = board[pawn_r][pawn_c]
                    if isinstance(p, Pawn) and p.color == 'black':
                        king_safety_score += self.PAWN_SHIELD_SCORE
                    elif p is None:
                        king_safety_score += self.PAWN_SHIELD_PENALTY_NO_PAWN
                    else:
                        king_safety_score += self.PAWN_SHIELD_PENALTY_MOVED_PAWN
            
            score -= king_safety_score # Black's king safety score is subtracted from total

        # The original comment: "Mobility and King Safety skipped for brevity in search to keep it FAST"
        # The above implementation re-adds these features.
    ...
    def get_board_hash(self, board, turn):
        ...
        # Include AI's simulation en passant target in the hash
        return (tuple(board_tuple), turn, self.ai_simulation_en_passant_target)
    ...
    def get_best_move(self, board, game_ref, ai_color):
        ...
        # Initialize AI's internal en passant target for this search
        self.ai_simulation_en_passant_target = game_ref.en_passant_target # Copy from the main game state
        ...
        try:
            ...
            for d in range(1, target_depth + 1):
                ...
                for start, end in all_moves:
                    ...
                    self.undo_move(search_board, move_info) # Pass the move_info dict
    ...
    def minimax(self, board, depth, alpha, beta, maximizing_player, game_ref):
        ...
        # Pass AI's internal en passant target to get_board_hash
        ...
        if depth == 0:
            eval = self.evaluate_board(board, game_ref) # Pass game_ref for mobility calculation
        ...
        if maximizing_player: 
            ...
            for i, (start, end) in enumerate(moves):
                ...
                self.undo_move(board, move_info) # Pass the move_info dict
        ...
        for r in range(8):
            for c in range(8):
                ...
                if piece and piece.color == color:
                    # Pass the AI's current simulated en_passant_target
                    valid_moves = game_ref.get_legal_moves(piece, (r, c), board, ep_target_for_check=self.ai_simulation_en_passant_target)
    ...
    def make_move(self, board, start, end):
        ...
        captured_piece_at_end_square = board[r2][c2] # This will be None for en passant capture target square
        captured_rook_info = None # For castling undo
        promoted_piece_original = None # If pawn promotes, store original pawn for undo
        en_passant_capture_info = None # (captured_pawn_object, captured_pawn_pos) for undo
        old_ai_simulation_en_passant_target = self.ai_simulation_en_passant_target # For undo

        # Handle En Passant Capture
        if isinstance(piece, Pawn) and end == self.ai_simulation_en_passant_target:
            # The captured pawn is one rank behind the destination square in the same column
            captured_pawn_pos = (r2 + 1, c2) if piece.color == 'white' else (r2 - 1, c2)
            captured_piece_obj = board[captured_pawn_pos[0]][captured_pawn_pos[1]]
            
            en_passant_capture_info = (captured_piece_obj, captured_pawn_pos)
            board[captured_pawn_pos[0]][captured_pawn_pos[1]] = None # Remove captured pawn
            # captured_piece_at_end_square remains None as the target square was empty
        
        # Handle Castling (Rook movement)
        if isinstance(piece, King) and abs(c2 - c1) == 2:
            # Determine rook's start and end positions
            if c2 == 6: # Kingside castling
                rook_start_c, rook_end_c = 7, 5
            else: # Queenside castling
                rook_start_c, rook_end_c = 0, 3
            
            rook_obj = board[r1][rook_start_c]
            if rook_obj: # Ensure it's actually a rook and not None
                captured_rook_info = (rook_obj, (r1, rook_start_c), rook_obj.has_moved)
                board[r1][rook_end_c] = rook_obj
                board[r1][rook_start_c] = None
                rook_obj.has_moved = True

        # Handle Promotion (AI always promotes to Queen for simplicity)
        piece_to_move_to_end = piece # Default: move the original piece
        if isinstance(piece, Pawn) and ((piece.color == 'white' and r2 == 0) or (piece.color == 'black' and r2 == 7)):
            promoted_piece_original = piece # Store original pawn object for undo
            piece_to_move_to_end = Queen(piece.color, 'Queen') # Create new Queen object for the target square

        # Perform the actual piece move
        board[r2][c2] = piece_to_move_to_end
        board[r1][c1] = None
        
        board[r2][c2].has_moved = True # The piece at the new position has now moved.

        # Calculate new en passant target for the simulated board state
        new_ai_simulation_en_passant_target = None
        if isinstance(piece, Pawn) and abs(r2 - r1) == 2: # Check original 'piece' for pawn type
            new_ai_simulation_en_passant_target = ((r1 + r2) // 2, c1)
        self.ai_simulation_en_passant_target = new_ai_simulation_en_passant_target
        
        # Return all necessary info for undo
        return {
            'start': start,
            'end': end,
            'moved_piece_obj': piece_to_move_to_end, # The piece object that landed on `end` (might be new Queen)
            'prev_has_moved': prev_has_moved, # For the 'original' piece (pawn, king, rook)
            'captured_at_end_square': captured_piece_at_end_square, # Piece at `end` square (None for EP)
            'captured_rook_info': captured_rook_info, # (rook_obj, old_rook_pos, old_rook_has_moved)
            'promoted_piece_original': promoted_piece_original, # Original pawn object if promotion occurred
            'en_passant_capture_info': en_passant_capture_info, # (captured_pawn_obj, captured_pawn_pos)
            'old_ai_simulation_en_passant_target': old_ai_simulation_en_passant_target # Old EP target
        }

    def undo_move(self, board, move_info): # Simplified call: just pass the returned dict
        start = move_info['start']
        end = move_info['end']
        moved_piece_obj = move_info['moved_piece_obj'] # The piece that was moved to 'end'
        prev_has_moved = move_info['prev_has_moved']
        captured_at_end_square = move_info['captured_at_end_square']
        captured_rook_info = move_info['captured_rook_info']
        promoted_piece_original = move_info['promoted_piece_original']
        en_passant_capture_info = move_info['en_passant_capture_info']
        old_ai_simulation_en_passant_target = move_info['old_ai_simulation_en_passant_target']

        r1, c1 = start
        r2, c2 = end
        
        # Restore the piece to its original position
        if promoted_piece_original:
            board[r1][c1] = promoted_piece_original # Put back the original pawn object
            # The 'moved_piece_obj' (the Queen) is essentially discarded
        else:
            board[r1][c1] = moved_piece_obj # Put the original piece back
        
        board[r1][c1].has_moved = prev_has_moved # Restore its has_moved status

        # Restore any captured piece at the end square (for normal captures)
        board[r2][c2] = captured_at_end_square

        # Undo Castling (restore rook)
        if captured_rook_info:
            rook_obj, old_rook_pos, old_rook_has_moved = captured_rook_info
            # Rook was at (r1, 5) or (r1, 3) (its temporary castled position)
            rook_current_pos_c = 5 if end[1] == 6 else 3 # Determine the column where rook was moved to
            board[r1][rook_current_pos_c] = None # Clear where rook was temporarily moved
            board[old_rook_pos[0]][old_rook_pos[1]] = rook_obj # Restore rook to original pos
            rook_obj.has_moved = old_rook_has_moved

        # Undo En Passant Capture (restore the captured pawn)
        if en_passant_capture_info:
            captured_pawn_obj, captured_pawn_pos = en_passant_capture_info
            board[captured_pawn_pos[0]][captured_pawn_pos[1]] = captured_pawn_obj # Put captured pawn back

        # Restore AI's internal en passant target
        self.ai_simulation_en_passant_target = old_ai_simulation_en_passant_target
...
class Game:
    ...
    def move(self, start, end):
        ...
        if isinstance(piece, Pawn) and (r2, c2) == self.en_passant_target:
            # The captured pawn is on the same column as the destination (c2)
            # and one rank behind the destination square (r2+1 for white attacker, r2-1 for black attacker)
            captured_pawn_pos = (r2 + 1, c2) if piece.color == 'white' else (r2 - 1, c2)
            
            # Get the captured pawn object for the half-move clock and sound
            captured_piece = self.board[captured_pawn_pos[0]][captured_pawn_pos[1]]
            self.board[captured_pawn_pos[0]][captured_pawn_pos[1]] = None # Remove the captured pawn
        else:
            # For normal captures, captured_piece is simply the piece on the target square
        ...
        # Perform the main piece move
        self.board[r2][c2] = piece
        self.board[r1][c1] = None
        piece.has_moved = True

        # Set En Passant Target for NEXT turn (only if pawn moved two squares)
        new_en_passant_target = None
        if isinstance(piece, Pawn) and abs(r2 - r1) == 2:
             new_en_passant_target = ((r1 + r2) // 2, c1)
        ...
        if isinstance(piece, Pawn):
            if (piece.color == 'white' and r2 == 0) or (piece.color == 'black' and r2 == 7):
                if self.turn == self.player_color:
                    ...
                    # AI pawn promotion (always to Queen for opponent)
                    # The piece variable in this scope is the pawn.
                    # We need to replace the piece on the board at (r2, c2)
                    self.board[r2][c2] = Queen(piece.color, 'Queen')
                    # The original 'piece' variable still refers to the Pawn object.

        # Play Move/Capture Sound
        if SOUND_ENABLED:
            if captured_piece: # This now correctly includes en passant captures
    ...
    def get_legal_moves(self, piece, pos, board, ep_target_for_check=None): # New parameter
        # Use the provided ep_target_for_check if available, otherwise use game's actual en_passant_target
        active_ep_target = ep_target_for_check if ep_target_for_check is not None else self.en_passant_target
        
        pseudo_moves = piece.get_valid_moves(pos, board, current_ep_target=active_ep_target) # Pass EP target
        legal_moves = []
        
        for move in pseudo_moves:
            # Pass the active_ep_target to simulate_move as well
            if not self.simulate_move(pos, move, piece.color, board, ep_target_before_sim_move=active_ep_target):
                legal_moves.append(move)
        
        # Castling Logic (remains unchanged, as EP doesn't affect castling)
                ...
                if p:
                    if p.color == enemy_color:
                        ...
                        if dist == 1 and isinstance(p, Pawn):
                            ...
                            # Check if the pawn is attacking diagonally towards 'pos'
                            if (pawn_dir == 1 and r == nr - 1 and abs(c - nc) == 1) or \
                               (pawn_dir == -1 and r == nr + 1 and abs(c - nc) == 1):
                                return True
    ...
    def simulate_move(self, start, end, color, board, ep_target_before_sim_move=None):
        r1, c1 = start
        r2, c2 = end
        piece = board[r1][c1]
        
        # Store temporary state for undo
        original_target_piece = board[r2][c2]
        
        captured_en_passant_piece = None
        captured_en_passant_pos = None

        # Handle En Passant Capture in simulation using ep_target_before_sim_move
        if isinstance(piece, Pawn) and end == ep_target_before_sim_move: # Use the provided EP target
            captured_en_passant_pos = (r2 + 1, c2) if piece.color == 'white' else (r2 - 1, c2)
            captured_en_passant_piece = board[captured_en_passant_pos[0]][captured_en_passant_pos[1]]
            board[captured_en_passant_pos[0]][captured_en_passant_pos[1]] = None # Remove captured pawn

        # Make the main move
        board[r2][c2] = piece
        board[r1][c1] = None
        
        # Check for check after the simulated move
        in_check = self.is_check(color, board)
        
        # Undo the move and state changes
        board[r1][c1] = piece
        board[r2][c2] = original_target_piece
        if captured_en_passant_piece:
            board[captured_en_passant_pos[0]][captured_en_passant_pos[1]] = captured_en_passant_piece
        ...
        for r in range(8):
            for c in range(8):
                ...
                if p and p.color == color:
                    # Pass None for ep_target_for_check here, as it's checking the *actual* board state
                    moves = self.get_legal_moves(p, (r, c), board, ep_target_for_check=None) 
                    if moves: 
                        return False
        return True
    ...
    def get_board_state_key(self):
        ...
        for r in range(8):
            for c in range(8):
                p = self.board[r][c]
                if p:
                    # Also include has_moved for castling/pawn moves
                    state += f"{p.name}{p.color}{r}{c}{'M' if p.has_moved else 'U'}" 
        return state
    ...
    def ai_move(self):
        ...
        def run_ai_search():
            try:
                ...
                # For Hard (depth 5) and Expert (depth 8) levels, set a 2-second time limit
                if self.ai.depth >= 5: # Includes both Hard (5) and Expert (8)
                    self.ai.search_start_time = start_time
                    self.ai.time_limit = 2.0 # Set to 2 seconds for Hard and Expert
                else: # Easy/Medium
                ...
                if self.ai.depth >= 5: # Hard and Expert difficulties
                    level_name = "Expert" if self.ai.depth >= 8 else "Hard"
                    print(f"{level_name} AI found move in {elapsed:.1f} seconds (responded immediately)")
                else: # Easy/Medium difficulties
                    # For Easy/Medium levels, ensure minimum 1 second response time
                    if elapsed < 1.0:
                        time.sleep(1.0 - elapsed)
                        print(f"AI took {1.0:.1f} seconds (minimum delay for Easy/Medium)")
                    else:
...
def draw_menu(win, font, current_difficulty, dropdown_open=False):
    ...
    title_rect = title_surf.get_rect(center=(WIDTH // 2, HEIGHT // 12 - 20)) # Moved up by 20 pixels
    ...
    draw_diff_btn(BTN_EXPERT_RECT, "Expert", 8) # Changed from 7 to 8
...
def draw(win, game, font):
    ...
    for r in range(8):
        for c in range(8):
            ...
            # Check if the current theme name contains "Wood" (case-insensitive)
            if "wood" in theme[2].lower(): 
                is_light = ((r + c) % 2 == 0)
                ...
                # Apply tint and grain only if not "Real Wood" theme's light square
                # or if it's the new "Dark Wood Original" for consistent texture application
                if not (theme[2] == "Real Wood" and is_light): # 'Dark Wood Original' is removed, so this condition simplifies
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if state == MENU:
                        ...
                        if BTN_EASY_RECT.collidepoint(event.pos):
                            ...
                            selected_difficulty = 8 # Expert difficulty remains at depth 8
                            print("Difficulty set to Expert (Depth 8 - Ultra Strength!)")
